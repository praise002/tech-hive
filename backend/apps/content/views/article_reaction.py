...
class ArticleReactionToggleView(APIView):
    """

    Toggle a reaction on an article.
    If user already reacted with this emoji, it removes it.
    If user hasn't reacted with this emoji, it adds it.

    User can have multiple different reactions on the same article.

    Requires authentication.
    """

    permission_classes = (IsAuthenticated,)
    serializer_class = ArticleReactionToggleSerializer
...
class ArticleReactionStatusView(APIView):
    """

    Get reaction status for an article.
    Returns counts per emoji type and which reactions the current user has used.

    Public endpoint - no authentication required.
    """

    serializer_class = ArticleReactionStatusSerializer
