...
class SubscriptionManager(models.Manager):
    ...
    def get_active_subscription(self, user):
        ...
        return self.select_related("user", "plan").filter(
            Q(status__in=["TRIALING", "ACTIVE", "PAST_DUE"])
            | Q(status="CANCELLED", current_period_end__gt=now),
            user=user,
        ).first()

    def get_latest_subscription(self, user):
        """Get user's most recent subscription (active or expired)"""
        return self.select_related("user", "plan").filter(user=user).order_by("-created_at").first()

    def active(self):
        """Get all active subscriptions (including trials and grace period)"""
        now = timezone.now()

        # Subscriptions with active statuses OR cancelled but still in paid period
    ...
    def expired(self):
        ...
        return self.select_related("user", "plan").filter(status="EXPIRED")

    def past_due(self):
        """Get subscriptions with failed payments"""
        return self.select_related("user", "plan").filter(status="PAST_DUE")

    def in_trial(self):
        """Get subscriptions currently in trial"""
        now = timezone.now()
        return self.select_related("user", "plan").filter(status="TRIALING", trial_end__gt=now)

    def trial_ending_soon(self, days=2):
        """Get trials ending within specified days"""
        now = timezone.now()
        end_date = now + timezone.timedelta(days=days)
        return self.select_related("user", "plan").filter(status="TRIALING", trial_end__range=(now, end_date))
