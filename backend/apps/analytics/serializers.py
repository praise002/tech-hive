...
# For tracking events from frontend
class TrackActivitySerializer(serializers.Serializer):
    ...
    device_type = serializers.ChoiceField(choices=DeviceTypeChoices.choices, required=False)
    ...
    load_time_ms = serializers.IntegerField(min_value=0, required=False, allow_null=True)
...
# For dashboard metrics response
...
class TopPerformingPostSerializer(serializers.Serializer):
...
# For article-specific analytics
